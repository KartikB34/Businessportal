...
function NavItem({Link}) {
    ...
    return (
        <div onClick={()=>{history.push(Link.link); setActiveNavItem(Link.id);}} className={`w-full flex items-center justify-start space-x-8 px-5 cursor-pointer group hover:border-gray-900 border-l-4 border-transparent ${activeNavItem===Link.id && "border-gray-900"}`}>
                    <span>{Link.icon}</span>
